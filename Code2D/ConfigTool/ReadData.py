...
def nifti_to_png(nifti_file_path: str, png_file_folder: str, cutting_dimension: int) -> list:
    '''
    从nifti文件中读取数据，沿指定维度(1，2，3)切片
    并保存为png文件。
    :param nifti_file_path: nifti文件路径
    :param png_file_folder: png文件保存路径
    :param cutting_dimension: 切片维度
    :return: 包含所有png文件地址的列表
    '''
    try:
        # 加载NIfTI文件
        nifti_image = nib.load(nifti_file_path)
        ...
        # 确保保存文件夹存在
        if not os.path.exists(png_file_folder):
